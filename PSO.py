# -*- coding: utf-8 -*-
# @Author: sadau
# @Date:   2021-04-02 10:27:49
# @Last Modified by:   sadau
# @Last Modified time: 2021-04-02 10:30:05
import numpy as np
...
#         self.w=1 #惯性权重
    ...
    def initpopvfit(self, sizepop):
        ...
        for i in range(sizepop):
            pop[i] = [(np.random.rand() - 0.5) * self.rangepop[0] * 2,
                      (np.random.rand() - 0.5) * self.rangepop[1] * 2]
            v[i] = [(np.random.rand() - 0.5) * self.rangepop[0] * 2,
                    (np.random.rand() - 0.5) * self.rangepop[1] * 2]
    ...
    def run(self):
        ...
        gbestpop, gbestfitness, pbestpop, pbestfitness = self.getinitbest(
            fitness, pop)
        ...
        for i in range(self.iter):
            ...
            for j in range(self.sizepop):
                v[j] = (0.5 * (100 - i) / 100 + 0.4) * v[j] + self.lr[0] * np.random.rand() * (
                    pbestpop[j] - pop[j]) + self.lr[1] * np.random.rand() * (gbestpop - pop[j])
